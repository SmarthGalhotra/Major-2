# DetectChars.py
import os #used for pause here

import cv2 #many uses
import numpy as np # Mainly used here to load text
import math # (math.sqrt) Used in Pythagoras Theorem and (atan, pi) to calculate angle between chars
import random # (randint() used to generate random blue, green and red. )

import Main # access 'showsteps' variable
import Preprocess #Preprocess possiblePlate
import PossibleChar # Find possible character in every contour


kNearest = cv2.ml.KNearest_create() #creates empty KNearest classifier. It should be then trained using StatModel::train method.
# https://docs.opencv.org/3.4/d5/d26/tutorial_py_knn_understanding.html

MIN_PIXEL_WIDTH = 2 #check If PossibleChar is of at least this pixel width
MIN_PIXEL_HEIGHT = 8
...
MIN_DIAG_SIZE_MULTIPLE_AWAY = 0.3 # Somewhere Used to check distance between chars
...
def loadKNNDataAndTrainKNN():
    ...
    try:
        ...
        os.system("pause")
        # program to wait until user hit enter so they can see their output.
        return False                                                                       
    # end try
